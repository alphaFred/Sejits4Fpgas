...
from ..nodes import CtreeNode, File
from .. import CONFIG
from ..util import singleton, highlight, truncate
from ..types import get_ctype, get_common_ctype, get_c_type_from_numpy_dtype
import hashlib
import ctypes


class CNode(CtreeNode):
    """Base class for all C nodes in vhdl_vhdl_ctree."""
    ...
    def label(self):
        from .dotgen import CDotGenLabeller
...
class CFile(CNode, File):
    ...
    def _compile(self, program_text):
        ...
        if recreate_so:
            ...
            CC = CONFIG.get(self.config_target, 'CC')
            CFLAGS = CONFIG.get(self.config_target, 'CFLAGS')
            LDFLAGS = CONFIG.get(self.config_target, 'LDFLAGS')
...
# mbd: deprecated. see vhdl_vhdl_ctree.cpp.nodes.CppDefine
