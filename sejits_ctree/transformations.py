...
import sejits_ctree
from sejits_ctree.c.nodes import Constant, String, SymbolRef, BinaryOp, TernaryOp, \
    Return, While, MultiNode, UnaryOp
from sejits_ctree.c.nodes import If, CFile, FunctionCall, FunctionDecl, For, Assign, \
    ArrayRef
from sejits_ctree.nodes import CtreeNode
from sejits_ctree.c.nodes import Lt, Gt, AddAssign
from sejits_ctree.c.nodes import Break, Continue, Pass, Array, Literal, And
from sejits_ctree.c.nodes import Op
from sejits_ctree.visitors import NodeTransformer

from sejits_ctree.types import get_common_ctype
...
class PyBasicConversions(NodeTransformer):
    ...
    def visit_AugAssign(self, node):
        ...
        if op in lookup:
            return getattr(sejits_ctree.c.nodes, lookup[op])(target, value)
        return node
...
class ResolveGeneratedPathRefs(NodeTransformer):
    """
    Converts any instances of sejits_ctree.nodes.GeneratedPathRef into strings
