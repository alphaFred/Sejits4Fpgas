...
from sejits_ctree.tools.generators import builder as Builder
...
def main(*args):
    '''run sejits_ctree utility stuff, currently only the project generator'''

    if sys.argv:
        args = sys.argv[1:]

    parser = argparse.ArgumentParser(prog="sejits_ctree", description="sejits_ctree is a python SEJITS framework")
    ...
    if args.startproject:
        ...
        from sejits_ctree.metrics.watts_up_reader import WattsUpReader
        ...
        sejits_ctree.CONFIG.set("jit", "CACHE", value="True")
        write_success = write_to_config('jit', 'CACHE', True)
        if write_success: print("[SUCCESS] sejits_ctree caching enabled.")

    elif args.disable_cache:
        wipe_cache()
        sejits_ctree.CONFIG.set("jit", "CACHE", value="False")
        write_success = write_to_config('jit', 'CACHE', False)
        args.clear_cache = True
        if write_success: print("[SUCCESS] sejits_ctree caching disabled.")
...
def get_responsible(section, key):
    ...
    """
    first = sejits_ctree.CFG_PATHS[-1]
    paths = reversed(sejits_ctree.CFG_PATHS)
    for path in paths:
...
def write_to_config(section, key, value):
    ...
    project, but does not write to the defaults.cfg file in sejits_ctree.
    :return: return True if write is successful. False otherwise.
    '''

    if sejits_ctree.CFG_PATHS:
        ...
        print("[FAILURE] No config file detected. Please create a '.sejits_ctree.cfg' file in your project directory.")
        return False

def wipe_cache():
    cache_name = os.path.expanduser(sejits_ctree.CONFIG.get('jit','COMPILE_PATH'))
