...
from sejits_ctree.nodes import CtreeNode, File
import sejits_ctree
from sejits_ctree.util import singleton, highlight, truncate
from sejits_ctree.types import get_ctype, get_common_ctype
import hashlib
import ctypes


class CNode(CtreeNode):
    """Base class for all C nodes in sejits_ctree."""

    def codegen(self, indent=0):
        from sejits_ctree.c.codegen import CCodeGen
        from sejits_ctree.transforms import DeclarationFiller

        return CCodeGen(indent).visit(self)

    def label(self):
        from sejits_ctree.c.dotgen import CDotGenLabeller
...
class CFile(CNode, File):
    ...
    def _compile(self, program_text):
        ...
        if recreate_so:
            ...
            CC = sejits_ctree.CONFIG.get(self.config_target, 'CC')
            CFLAGS = sejits_ctree.CONFIG.get(self.config_target, 'CFLAGS')
            LDFLAGS = sejits_ctree.CONFIG.get(self.config_target, 'LDFLAGS')
...
# mbd: deprecated. see sejits_ctree.cpp.nodes.CppDefine
