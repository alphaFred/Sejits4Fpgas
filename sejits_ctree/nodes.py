...
class CtreeNode(ast.AST):
    """Base class for all AST nodes in sejits_ctree."""
    _fields = []
...
class Project(CommonNode):
    ...
    def codegen(self, indent=0):
        ...
        """
        from sejits_ctree.jit import JitModule

        self._module = JitModule()

        # now that we have a concrete compilation dir, resolve references to it
        from sejits_ctree.transformations import ResolveGeneratedPathRefs
        #
...
class File(CommonNode):
    ...
    def __init__(self, name="generated", body=None, path = None):
        ...
        self.path = path or sejits_ctree.CONFIG.get('jit','COMPILE_PATH')
