...
def main(*args):
    """run vhdl_vhdl_ctree utility stuff, currently only the project generator"""

    if sys.argv:
        args = sys.argv[1:]

    parser = argparse.ArgumentParser(prog="vhdl_vhdl_ctree", description="vhdl_vhdl_ctree is a python SEJITS framework")
    ...
    if args.startproject:
        ...
        from metrics.watts_up_reader import WattsUpReader
        ...
        CONFIG.set("jit", "CACHE", value="True")
        write_success = write_to_config('jit', 'CACHE', True)
        if write_success:
            print("[SUCCESS] vhdl_vhdl_ctree caching enabled.")

    elif args.disable_cache:
        wipe_cache()
        CONFIG.set("jit", "CACHE", value="False")
        write_success = write_to_config('jit', 'CACHE', False)
        args.clear_cache = True
        if write_success:
            print("[SUCCESS] vhdl_vhdl_ctree caching disabled.")
...
def write_to_config(section, key, value):
    ...
    project, but does not write to the defaults.cfg file in vhdl_vhdl_ctree.
    ...
    if CFG_PATHS:
        ...
        print("[FAILURE] No config file detected. Please create a '.vhdl_vhdl_ctree.cfg' file in your project directory.")
        return False
