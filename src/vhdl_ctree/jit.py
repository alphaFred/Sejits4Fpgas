...
from .nodes import Project
from .analyses import VerifyOnlyCtreeNodes
from .frontend import get_ast, dump
from .transforms import DeclarationFiller
from .c.nodes import CFile, MultiNode
if OCL_ENABLED:
    from .ocl.nodes import OclFile
...
class JitModule(object):
    ...
    def __init__(self):
        import os
        if CONFIG.get('jit', 'COMPILE_PATH') and CONFIG.getboolean('jit', 'CACHE'):
            ctree_dir = CONFIG.get('jit','COMPILE_PATH')
...
class LazySpecializedFunction(object):
    ...
    def config_to_dirname(self, program_config):
        ...
        compile_path = str(CONFIG.get('jit', 'COMPILE_PATH'))
    ...
    def get_transform_result(self, program_config, dir_name, cache=True):
        ...
        if hash(self) != info['hash'] and self.original_tree is not None \
            ...
            STATS.log("Filesystem cache miss")
            ...
            STATS.log('Filesystem cache hit')
    ...
    def __call__(self, *args, **kwargs):
        ...
        """
        STATS.log("specialized function call")
    ...
    def get_tuning_driver(self):
        ...
        """
        from .tune import ConstantTuningDriver
