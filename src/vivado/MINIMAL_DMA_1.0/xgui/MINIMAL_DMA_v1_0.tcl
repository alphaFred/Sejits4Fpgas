...
proc init_gui { IPINST } {
  ...
  ipgui::add_param $IPINST -name "EN_WRITE" -parent ${Page_0}
  ipgui::add_param $IPINST -name "C_M_AXI_DATA_WIDTH" -parent ${Page_0} -widget comboBox
  ipgui::add_param $IPINST -name "C_M_AXI_BURST_LEN" -parent ${Page_0} -widget comboBox
  ipgui::add_param $IPINST -name "LEN_WIDTH" -parent ${Page_0}
  set ISSUE_DEPTH [ipgui::add_param $IPINST -name "ISSUE_DEPTH" -parent ${Page_0}]
  set_property tooltip {Number of parallel requests} ${ISSUE_DEPTH}


}

proc update_PARAM_VALUE.EN_WRITE { PARAM_VALUE.EN_WRITE } {
	# Procedure called to update EN_WRITE when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.EN_WRITE { PARAM_VALUE.EN_WRITE } {
	# Procedure called to validate EN_WRITE
	return true
}

proc update_PARAM_VALUE.ISSUE_DEPTH { PARAM_VALUE.ISSUE_DEPTH } {
	# Procedure called to update ISSUE_DEPTH when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.ISSUE_DEPTH { PARAM_VALUE.ISSUE_DEPTH } {
	# Procedure called to validate ISSUE_DEPTH
	return true
}
...
proc update_MODELPARAM_VALUE.ISSUE_DEPTH { MODELPARAM_VALUE.ISSUE_DEPTH PARAM_VALUE.ISSUE_DEPTH } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.ISSUE_DEPTH}] ${MODELPARAM_VALUE.ISSUE_DEPTH}
}
